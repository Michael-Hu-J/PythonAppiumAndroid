...
class MallGoods(Base):
    def add_goods(self):
        super().swipe_find_element(element_locate="//android.widget.TextView[@text='商品管理']").click()
        super().click_element(element_locate="//android.widget.TextView[@text='分类管理']", page_description="点击分类管理")
        super().click_element(element_locate="//android.widget.TextView[@text='商品分类']", page_description="点击添加商品分类")

        # 混合定位，两个属性值均属于同一个元素
        super().find_element_android_uiautomator(
        ...
        super().click_element(element_locate="//android.widget.TextView[@text='商品信息']", page_description="点击商品信息")
        super().click_element(element_locate="//android.widget.TextView[@text='添加']", page_description="点击添加商品")
        super().click_element(
        ...
        while text != "自动化测试分类":
            ...
            # 一次刚好滑动一个分类
            super().swipe(start_x=0.5, start_y=0.8, end_x=0.5, end_y=0.77, duration=500)
            super().click_element(
                element_locate="//android.widget.TextView[@resource-id='zmsoft.rest.phone:id/btn_confirm']",
                page_description="点击确认商品分类")
            # 获取商品分类当前已选择的分类名称
            text = super().find_element_android_uiautomator(
                ui_selector='new UiSelector().text("商品分类").fromParent(resourceId("zmsoft.rest.phone:id/detail"))',
        ...
        super().click_element(element_locate="//android.widget.TextView[@text='保存']", page_description="点击保存商品")

    def delete_goods(self):
        super().swipe_find_element(element_locate="//android.widget.TextView[@text='商品管理']").click()
        super().click_element(element_locate="//android.widget.TextView[@text='商品信息']", page_description="点击商品信息")
        super().click_element(element_locate="//android.widget.TextView[@text='自动化测试商品']", page_description="点击商品")
        ...
        super().click_element(element_locate="//android.widget.TextView[@text='删除']", page_description="点击删除商品")
        super().click_element(element_locate="//android.widget.Button[@text='确定']", page_description="点击确定删除商品")
        super().click_element(
            element_locate="//android.widget.LinearLayout[@resource-id='zmsoft.rest.phone:id/layout_left']",
            page_description="点击返回到商品管理页")
        super().click_element(element_locate="//android.widget.TextView[@text='分类管理']", page_description="点击分类管理")
        super().click_element(element_locate="//android.widget.TextView[@text='自动化测试分类']", page_description="点击商品分类")
        super().click_element(element_locate="//android.widget.TextView[@text='删除']", page_description="点击删除商品分类")
        super().click_element(element_locate="//android.widget.Button[@text='确定']", page_description="点击确定删除商品分类")
